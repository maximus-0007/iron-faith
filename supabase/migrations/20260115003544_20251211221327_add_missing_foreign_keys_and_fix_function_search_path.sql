...
-- ============================================================================
-- ADD MISSING FOREIGN KEY CONSTRAINTS (only for tables that don't have them yet)
-- ============================================================================

-- Add foreign key for conversation_folders.user_id (if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Add foreign key for message_bookmarks.user_id (if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
